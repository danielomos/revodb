...
@RestController
@RequestMapping("/api/revogasgateway-kafka")
public class RevogasgatewayKafkaResource {

    private final Logger log = LoggerFactory.getLogger(RevogasgatewayKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public RevogasgatewayKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
